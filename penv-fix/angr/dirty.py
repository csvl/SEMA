...
def x86g_dirtyhelper_CPUID_sse3(state, _):
    if 'CPUID_SYMBOLIC' in state.options:
        state.regs.eax = claripy.BVS("cpuid_a", 32)
        state.regs.ebx = claripy.BVS("cpuid_b", 32)
        state.regs.ecx = claripy.BVS("cpuid_c", 32)
    ...
    if state.arch.name == 'X86':
        ...
        SET_ABCD(0x80000001, 0x786e6547, 0x49656e69, 0x6c65746e, 0)
        SET_ABCD(0x80000008, 0x00003024, 0x00000000, 0x00000000, 0)

    return None, [ ]

x86g_dirtyhelper_CPUID_sse2 = x86g_dirtyhelper_CPUID_sse0
x86g_dirtyhelper_CPUID_sse3 = x86g_dirtyhelper_CPUID_sse3
