...
class SVMClassifier(Classifier):
    def __init__(self,path,name, threshold=0.45, 
        ...
        self.mapping = self.read_mapping(self.path + '/mapping_' + input_dir_name + '.txt')
        self.mapping_inv = self.read_mapping_inverse(self.path + '/mapping_' + input_dir_name + '.txt')
        self.dataset = []
        self.label = []
        self.fam_idx = []
    ...
    def init_dataset(self, path):
        ...
        for family in self.families:
            path = self.path.replace("/sema_classifier","/") + '/'  + self.original_path + family + '/' if self.path.replace("/sema_classifier","/") not in self.original_path else self.original_path + family + '/'
            path = path.replace("sema_classifier/","").replace("/src/src/","/src/") # TODO refactor
