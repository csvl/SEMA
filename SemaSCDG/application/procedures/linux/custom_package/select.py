    ...
    def run(self, nfds, readfds, writefds, exceptfds, timeout):
        try:
            ...
            return self.state.solver.Unconstrained(
                "unconstrained_ret_%s" % "select",
                self.state.arch.bits,
                key=("api", "?", "select"),
            )
            raise angr.errors.SimProcedureArgumentError(
                "Can't handle symbolic select arguments"
            )

        if writefds_v != 0 or exceptfds_v != 0:
            # return self.state.solver.Unconstrained("unconstrained_ret_%s" % 'select', self.state.arch.bits, key=('api', '?', 'select'))
        ...
        for offset in range(0, long_array_size):
            long = self.state.memory.load(
                readfds + offset * arch_bytes,
                arch_bytes,
        ...
        retval = self.state.solver.BVV(0, 1).concat(
            self.state.solver.BVS("select_ret", 31)
        )
        return retval
