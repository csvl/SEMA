...
class MurmurHashHook(angr.SimProcedure):
    NO_RET = True
    ...
    def run(self):
        ...
        hashh = mmh3.hash(bytestring,self.state.solver.eval(y),False)
        ptr = self.state.solver.BVV(hashh,32)
        self.state.memory.store(z,ptr,endness=self.state.arch.memory_endness)
