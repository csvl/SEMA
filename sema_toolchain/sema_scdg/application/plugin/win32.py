...
from angr.plugin import SimStatePlugin
from angr.filesystem import SimMount
...
class Win32DevFS(SimMount): # this'll be mounted at /dev
    def get(self, path): # pylint: disable=arguments-differ
        if path == ['stdin']:
            return self.state.win32.fd.get(0, None)
        elif path == ['stdout']:
            return self.state.win32.fd.get(1, None)
        elif path == ['stderr']:
            return self.state.win32.fd.get(2, None)
        else:
            return None
...
class SimSystemWin32(SimStatePlugin):
    """
    ...
    def _pick_fd(self):
        for fd in range(0, 8192):
            if fd not in self.fd:
                return fd
        raise SimWin32Error('exhausted file descriptors')
    ...
    def fstat(self, sim_fd): #pylint:disable=unused-argument
        ...
        if fd is not None:
            fd_desc = self.state.win32.get_fd(fd)
        ...
        if guest_path is not None and mount is not None:
            ...
            if stat is None:
                raise SimWin32Error("file %s does not exist on mount %s" % (guest_path, mount))
            size = stat.st_size
            mode = stat.st_mode
            ino = stat.st_ino
        else:
        ...
        return FileTime(self.state.solver.BVV(0, 64), # st_dev
...
SimState.register_default('win32', SimSystemWin32)

from ..errors import SimWin32Error, SimSolverError, SimMergeError, SimMemoryError
